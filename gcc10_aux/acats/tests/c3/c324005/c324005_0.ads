...
--     assertion policy is Ignore). Case A: specific assertion policy pragmas.
--
--     Check that a static or dynamic predicate specified directly for a
...
--
-- CHANGE HISTORY:
--     14 May 15   RLB     Created test based on C324004.
--
with Ada.Assertions; use Ada.Assertions;
package C324005_0 is

   pragma Assertion_Policy (Check);
   pragma Assertion_Policy (Static_Predicate  => Ignore,
                            Dynamic_Predicate => Ignore);
   -- Predicate checks (but not others) are disabled for the
