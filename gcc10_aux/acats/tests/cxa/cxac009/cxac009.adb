...
with Report;
procedure CXAC009 is
   Log_File_Name : constant String :=
      Report.Legal_File_Name (Nam => "CXAC009");
      -- A file name that will be used for the log file. This file will
      -- be written.
begin
   Report.Test ("CXAC009", "Check that Stream_IO.File_Type has " &
   ...
   CXAC009_Logger.Open_Log (Log_File_Name);

   CXAC009_Logger.Log_Item ("Start");

   CXAC009_Test.Do_Something;

   CXAC009_Logger.Log_Item ("End");

   CXAC009_Logger.Close_Log;
   ...
   declare
      ...
      begin
         ...
         if Last /= Data'Length+2 then
             Report.Failed ("Wrong length read, premature end of file for " &
                            Data);
         end if;
