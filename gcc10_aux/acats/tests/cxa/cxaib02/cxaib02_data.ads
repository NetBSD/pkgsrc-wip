...
--
-- OBJECTIVE:
--      Check that Ada.Containers.Bounded_Vectors is preelaborated.
--
--      Check that type Vector from an instance of
--      Ada.Containers.Bounded_Vectors has Preeelaborable_Initialization if
--      the element type has Preelaborable_Initialization.
--
--      Check that an instance of Ada.Containers.Bounded_Vectors can be
...
--      keeps its data in instances of Ada.Containers.Bounded_Vectors. Static
--      items are kept in one vector and movable items are kept in a different
--      vector.
--
--      The data are declared in a preelaborated package to improve startup
--      times.
--
--      AI12-0409-1 says that a bounded Vector has preelaborable initialization
--      if and only if the element type has preelaborable initialization.
--      But a bounded Vector should always be usable in contexts where
...
package CXAIB02_Data
   with Preelaborate is

   package Static_Data is new Ada.Containers.Bounded_Vectors (Positive,
                                                      FXAIB00.Location,
                                                      FXAIB00."=");

   Static_Locations : Static_Data.Vector(10);
   ...
   package Moveable_Data is new
                       Ada.Containers.Bounded_Vectors (Positive,
