...
--- gdbserver/netbsd-low.cc.orig	2020-08-25 22:25:31.585096805 +0000
...
+
+int
+netbsd_process_target::read_memory (CORE_ADDR memaddr, unsigned char *myaddr, int size)
+{
...
+
+int
+netbsd_process_target::write_memory (CORE_ADDR memaddr, const unsigned char *myaddr, int size)
+{
+  netbsd_debug ("%s(memaddr=%p, myaddr=%p, size=%d)\n",
+                __func__, memaddr, myaddr, size);
+
...
+
+int
+netbsd_process_target::qxfer_libraries_svr4 (const char *annex, unsigned char *readbuf,
