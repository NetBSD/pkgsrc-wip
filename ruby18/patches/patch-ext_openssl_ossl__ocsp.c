...
@@ -103,15 +103,17 @@ ossl_ocspreq_initialize(int argc, VALUE *argv, VALUE s
...
@@ -134,7 +136,7 @@ ossl_ocspreq_add_nonce(int argc, VALUE *argv, VALUE se
     else{
 	StringValue(val);
 	GetOCSPReq(self, req);
...
@@ -310,15 +312,17 @@ ossl_ocspres_initialize(int argc, VALUE *argv, VALUE s
...
+    p = (unsigned char *)RSTRING_PTR(str);
 	...
 	ossl_raise(eOCSPError, NULL);
     ossl_str_adjust(str, p);
@@ -436,7 +440,7 @@ ossl_ocspbres_add_nonce(int argc, VALUE *argv, VALUE s
     else{
 	StringValue(val);
